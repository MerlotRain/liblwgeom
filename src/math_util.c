...
double
math_to_degrees (double radians)
{
  return radians * 180.0 / M_PI;
}

double
math_radians (double degrees)
{
  return degrees * M_PI / 180.0;
}

double
math_angle (double x1, double y1, double x2, double y2)
{
  double dx = x2 - x1;
  double dy = y2 - y1;
  return atan2 (dy, dx);
...
int
math_is_acute (double x1, double y1, double x2, double y2, double x3,
               double y3)
{
  double dx0 = x1 - x2;
  double dy0 = y1 - y2;
...
double
math_distance (double x1, double y1, double x2, double y2)
{
  double dx = x1 - x2;
  double dy = y1 - y2;
...
double
math_ccw_angle (double dy, double dx)
{
...
double
math_sweep_angle (double cx, double cy, double x1, double y1, double x2,
...
void
math_circle_center_radius (double x1, double y1, double x2, double y2,
                           double x3, double y3, double *radius, double *cx,
                           double *cy)
{
...
double
math_circle_length (double x1, double y1, double x2, double y2, double x3,
                    double y3)
{
  double cx;
  double cy;
...
static void
__math_bezier_split (bezier_t *bezier, bezier_t *halves)
{
  ...
  raw_point_t mid_12_23
      = { .x = (mid_12.x + mid_23.x) * 0.5, .y = (mid_12.y + mid_23.y) * 0.5 };
  raw_point_t mid_23_34
      = { .x = (mid_23.x + mid_34.x) * 0.5, .y = (mid_23.y + mid_34.y) * 0.5 };
...
__math_bezier_add_if_close (bezier_t *bezier, double *length, double error)
{
  ...
  len += math_distance (bezier->p1.x, bezier->p1.y, bezier->p2.x,
                        bezier->p2.y);
  len += math_distance (bezier->p2.x, bezier->p2.y, bezier->p3.x,
                        bezier->p3.y);
  len += math_distance (bezier->p3.x, bezier->p3.y, bezier->p4.x,
                        bezier->p4.y);
...
double
math_bezier_length (double x1, double y1, double x2, double y2, double x3,
