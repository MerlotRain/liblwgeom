...
static char *__next_word (stok *token);

static void __get_coordinates (stok *token, double **coordinates, int *num);

static char *__get_next_empty_or_opener (stok *token);

se_geom *
geom_read_wkt (const char *data, int len)
{
#ifdef _MSC_VER
  ...
  char *type = __next_word (&token, &flags);
...
static void
__get_coordinates (stok *token, double **coordinates, int *num)
{
  char *nexttok = __get_next_empty_or_opener (token);
}

char *
__get_next_empty_or_opener (stok *token)
{
  char *nextword = __next_word (token);
  if (strcmp (nextword, "ZM") == 0)
    {
      nextword = __next_word (token);
    }
  else
    {
