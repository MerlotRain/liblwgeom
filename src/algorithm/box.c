...
bool mg_box_intersects(const struct nv_box env1, const struct nv_box env2)
{
...
struct nv_box mg_box_intersection(const struct nv_box env1,
...
struct nv_box mg_box_union(const struct nv_box env1, const struct nv_box env2)
{
    MG_BOX_INIT(b)
    if (MG_BOX_NULL(env1) || MG_BOX_NULL(env2)) {
        return b;
...
struct nv_geobject *mg_stroke_box(struct nv_box e, int gdim)
{
}
