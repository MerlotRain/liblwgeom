...
{
  se_inthash *hash = (se_inthash *)malloc (sizeof (se_inthash));
  uint64_t cap;
  if (!hash)
    return NULL;
...
static inline int
hash_need_resize (se_inthash *hash)
{
  return (uint64_t)(hash->elems * 100.0 / HASH_LOAD) > hash->size;
}

static void
hash_resize (se_inthash *hash)
{
  se_inthash oldhash = *hash;
...
uint64_t
hash_get (se_inthash *hash, uint64_t key)
{
...
void
hash_set (se_inthash *hash, uint64_t key, uint64_t value)
{
  uint64_t i = hash_func (key) % hash->size;
  uint64_t j = i;
  if (key == 0)
