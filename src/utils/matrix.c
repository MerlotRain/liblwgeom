...
void
matrix_left_multiply (matrix_t *m, const matrix_t *mat)
{
...
void
matrix_multiply (matrix_t *m, const matrix_t *mat)
{
  double mm[6]
      = { m->m_m[0], m->m_m[1], m->m_m[2], m->m_m[3], m->m_m[4], m->m_m[5] };
...
void
matrix_translate (matrix_t *m, const float x, const float y)
{
  m->m_dx += x;
  m->m_dy += y;
}
...
void
matrix_scale (matrix_t *m, const float x, const float y)
{
  m->m_m11 *= x;
  m->m_m21 *= x;
  m->m_dx *= x;
...
void
matrix_rotate (matrix_t *m, const float angle)
{
  matrix_rotate2 (m, angle, 0, 0);
}

void
matrix_rotate2 (matrix_t *m, const float angle, const float x, const float y)
{
...
void
matrix_shear (matrix_t *m, const float x, const float y)
{
  double mm[6]
      = { m->m_m[0], m->m_m[1], m->m_m[2], m->m_m[3], m->m_m[4], m->m_m[5] };
...
void
matrix_mirror (matrix_t *m, const float x, const float y)
{
  double mm[6]
      = { m->m_m[0], m->m_m[1], m->m_m[2], m->m_m[3], m->m_m[4], m->m_m[5] };
...
bool
matrix_invertible (const matrix_t *m)
{
...
bool
matrix_identity (const matrix_t *m)
{
...
void
matrix_trans_points (const matrix_t *m, double *points, int count)
{
