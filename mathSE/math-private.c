...
double math_circle_length(double x1, double y1, double x2, double y2, double x3,
  ...
  double length =
      M_PI / 180.0 * radius * math_sweep_angle(cx, cy, x1, y1, x2, y2, x3, y3);
  if (length < 0) {
    length = -length;
  }
  return length;
...
static void __math_bezier_add_if_close(bezier_t *bezier, double *length,
                                       double error) {
  double len = 0.0;
  double chord;

  len += math_distance(bezier->p1.x, bezier->p1.y, bezier->p2.x, bezier->p2.y);
  len += math_distance(bezier->p2.x, bezier->p2.y, bezier->p3.x, bezier->p3.y);
  len += math_distance(bezier->p3.x, bezier->p3.y, bezier->p4.x, bezier->p4.y);

  chord = math_distance(bezier->p1.x, bezier->p1.y, bezier->p4.x, bezier->p4.y);
